...
def _run_single(seed: int, steps: int, p: float = 0.001, enable_topology: bool | None = None) -> Dict[str, Any]:
    regime_seq = _generate_regime_sequence(p=p, seed=int(seed), length=int(steps))
    world = AdversarialPhaseShiftBanditWorld(
        p=float(p),
        sigma_phase1=0.005,
        sigma_active_high=0.02,
        sigma_opposing_low=0.005,
        sigma_other=0.01,
        seed=int(seed),
        horizon=int(steps),
        phase_split=int(steps) // 2,
        regime_sequence=regime_seq,
    )

    selector = CapitalSelectorBuilder().with_K(0).build()
    stack_manager = StackManager(world_id="regime", phase_id="G3_4_12", run_id=f"run_{seed}")
    ...
    return {
        ...
        "regime_seq": list(regime_seq),
    }


def run_g3_4_12_sweep(
    seeds: Iterable[int], steps: int = 500, p: float = 0.001, enable_topology: bool | None = None
) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for seed in seeds:
        results.append(_run_single(int(seed), int(steps), p=float(p), enable_topology=enable_topology))
    return results
