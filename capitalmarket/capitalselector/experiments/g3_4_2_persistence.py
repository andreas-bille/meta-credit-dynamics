...
def _run_single(p: float, seed: int, steps: int, enable_topology: bool | None = None) -> Dict[str, Any]:
    world = RegimeSwitchBanditWorld(p=p, sigma=0.01, seed=int(seed), c_high=0.0)
    selector = CapitalSelectorBuilder().with_K(0).build()
    stack_manager = StackManager(world_id="regime", phase_id="G3_4_2", run_id=f"run_{seed}_{p}")
...
def run_g3_4_2_sweep(p_values: Iterable[float], seeds: Iterable[int], steps: int = 500, enable_topology: bool | None = None) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for p in p_values:
        for seed in seeds:
            results.append(_run_single(float(p), int(seed), int(steps), enable_topology=enable_topology))
    return results
