...
from ..worlds.regime_switch_bandit_world import RegimeSwitchBanditWorld, VolatilityRegimeBanditWorld
...
def _run_world(world, *, steps: int, run_id: str, enable_topology: bool | None = None) -> Dict[str, Any]:
    ...
    stack_manager = StackManager(world_id="regime", phase_id="G3_4_3", run_id=run_id)
...
def run_g3_4_3_sweep(p_values: Iterable[float], seeds: Iterable[int], steps: int = 500, enable_topology: bool | None = None) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for p in p_values:
        for seed in seeds:
            baseline = RegimeSwitchBanditWorld(p=float(p), sigma=0.01, seed=int(seed), c_high=0.0)
            vol = VolatilityRegimeBanditWorld(p=float(p), sigma_low=0.005, sigma_high=0.02, seed=int(seed))
            baseline_result = _run_world(baseline, steps=int(steps), run_id=f"baseline_{seed}_{p}", enable_topology=enable_topology)
            vol_result = _run_world(vol, steps=int(steps), run_id=f"vol_{seed}_{p}", enable_topology=enable_topology)
            results.append(
                {
                    "p": float(p),
                    "seed": int(seed),
