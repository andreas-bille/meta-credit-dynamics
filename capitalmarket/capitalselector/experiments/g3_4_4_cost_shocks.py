...
def _run_single(p: float, q: float, seed: int, steps: int, c_spike: float = 0.05, enable_topology: bool | None = None) -> Dict[str, Any]:
    world = RegimeSwitchBanditWorld(
        p=p, sigma=0.01, seed=int(seed), q=float(q), c_spike=float(c_spike)
    )
    selector = CapitalSelectorBuilder().with_K(0).build()
    stack_manager = StackManager(world_id="regime", phase_id="G3_4_4", run_id=f"run_{seed}_{p}_{q}")
...
def run_g3_4_4_sweep(
    p_values: Iterable[float], q_values: Iterable[float], seeds: Iterable[int], steps: int = 500, enable_topology: bool | None = None
) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for p in p_values:
        for q in q_values:
            for seed in seeds:
                results.append(_run_single(float(p), float(q), int(seed), int(steps), enable_topology=enable_topology))
    return results
