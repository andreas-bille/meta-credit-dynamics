...
from ..worlds.regime_switch_bandit_world import RegimeSwitchBanditWorld, SubsetRegimeBanditWorld
...
def _run_world(world, *, steps: int, run_id: str, enable_topology: bool | None = None) -> Dict[str, Any]:
    ...
    stack_manager = StackManager(world_id="regime", phase_id="G3_4_5", run_id=run_id)
    ...
    for t in range(int(steps)):
        ...
        if w is None:
            ...
            weight_share_4 = float(w[4]) if len(w) > 4 else 0.0
    ...
    return {
        ...
        "aggregates": {
            ...
            "weight_share_4": float(np.mean([o["weight_share_4"] for o in observables])) if observables else 0.0,
        },
        "r_stats": {"mean": r_mean},
        "dumps": dumps,
    }
...
def run_g3_4_5_sweep(p_values: Iterable[float], seeds: Iterable[int], steps: int = 500, enable_topology: bool | None = None) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for p in p_values:
        for seed in seeds:
            subset_world = SubsetRegimeBanditWorld(p=float(p), sigma=0.01, seed=int(seed))
            baseline_world = RegimeSwitchBanditWorld(p=float(p), sigma=0.01, seed=int(seed), c_high=0.0)
            subset = _run_world(subset_world, steps=int(steps), run_id=f"subset_{seed}_{p}", enable_topology=enable_topology)
            baseline = _run_world(baseline_world, steps=int(steps), run_id=f"baseline_{seed}_{p}", enable_topology=enable_topology)
            results.append(
                {
                    "p": float(p),
                    "seed": int(seed),
