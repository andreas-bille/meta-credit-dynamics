...
SHOCK_TIMES = {100, 200, 300, 400}


def _run_single(p: float, mode: str, seed: int, steps: int, shock_size: float = 0.2, enable_topology: bool | None = None) -> Dict[str, Any]:
    shock_times = SHOCK_TIMES if mode == "shock" else set()
    ...
    stack_manager = StackManager(world_id="regime", phase_id="G3_4_8", run_id=f"run_{seed}_{p}_{mode}")
...
def run_g3_4_8_sweep(
    p_values: Iterable[float], modes: Iterable[str], seeds: Iterable[int], steps: int = 500, enable_topology: bool | None = None
) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for p in p_values:
        for mode in modes:
            for seed in seeds:
                results.append(_run_single(float(p), str(mode), int(seed), int(steps), enable_topology=enable_topology))
    return results
