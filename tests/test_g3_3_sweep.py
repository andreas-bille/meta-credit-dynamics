...
from capitalmarket.capitalselector.experiments.g3_3_sweep import run_g3_3_sweep
...
def test_g3_3_determinism_per_pair():
    p_values = [0.01, 0.05, 0.1]
    seeds = [0, 1, 2, 3, 4]
    results1 = run_g3_3_sweep(p_values, seeds, steps=500)
    results2 = run_g3_3_sweep(p_values, seeds, steps=500)
    assert len(results1) == len(results2) == 15
...
def test_g3_3_finite_outputs():
    p_values = [0.01, 0.05, 0.1]
    seeds = [0, 1, 2, 3, 4]
    results = run_g3_3_sweep(p_values, seeds, steps=500)
    for r in results:
        assert np.isfinite(r["aggregates"]["final_wealth"])


def test_g3_3_no_semantic_mutation():
    selector = CapitalSelectorBuilder().with_K(0).build()
    stack_manager = StackManager(world_id="regime", phase_id="G3_3", run_id="run_0")
