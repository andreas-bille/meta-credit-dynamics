...
from capitalmarket.capitalselector.experiments.g3_4_11_flip_sediment import run_g3_4_11_flip_sediment
...
from capitalmarket.capitalselector.worlds.flip_cluster_world import FlipClusterWorld
...
def test_g3_4_11_determinism(monkeypatch):
    monkeypatch.setenv("CAPM_ENABLE_TOPOLOGY", "1")
    result1 = run_g3_4_11_flip_sediment(steps=500, flip_time=250)
    result2 = run_g3_4_11_flip_sediment(steps=500, flip_time=250)
...
@pytest.mark.xfail(reason="Sediment not yet observed under current flip/shock settings.")
def test_g3_4_11_sediment_creation(monkeypatch):
    monkeypatch.setenv("CAPM_ENABLE_TOPOLOGY", "1")
    result = run_g3_4_11_flip_sediment(steps=500, flip_time=250)
    assert result["aggregates"]["final_sediment_count"] > 0


def test_g3_4_11_no_semantic_mutation():
    ...
    world = FlipClusterWorld(flip_time=250)
    out = world.step(0)
