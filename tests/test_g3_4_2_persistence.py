...
from capitalmarket.capitalselector.experiments.g3_4_2_persistence import run_g3_4_2_sweep
...
def test_g3_4_2_determinism():
    p_values = [0.001, 0.01]
    seeds = [0, 1, 2, 3, 4]
    results1 = run_g3_4_2_sweep(p_values, seeds, steps=500)
    results2 = run_g3_4_2_sweep(p_values, seeds, steps=500)
...
def test_g3_4_2_finite_outputs():
    p_values = [0.001, 0.01]
    seeds = [0, 1, 2, 3, 4]
    results = run_g3_4_2_sweep(p_values, seeds, steps=500)
    for r in results:
        assert np.isfinite(r["aggregates"]["final_wealth"])


def test_g3_4_2_no_semantic_mutation():
    selector = CapitalSelectorBuilder().with_K(0).build()
    stack_manager = StackManager(world_id="regime", phase_id="G3_4_2", run_id="run_0")
    dump_before = canonical_state_dump(selector, stack_manager=stack_manager, sediment=stack_manager.sediment)

    world = RegimeSwitchBanditWorld(p=0.01, sigma=0.01, seed=1, c_high=0.0)
    out = world.step(0)
