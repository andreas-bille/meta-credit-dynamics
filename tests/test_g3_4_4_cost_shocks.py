...
from capitalmarket.capitalselector.experiments.g3_4_4_cost_shocks import run_g3_4_4_sweep
...
def test_g3_4_4_determinism():
    p_values = [0.01, 0.05]
    q_values = [0.0, 0.02]
    seeds = [0, 1, 2, 3, 4]
    results1 = run_g3_4_4_sweep(p_values, q_values, seeds, steps=500)
    results2 = run_g3_4_4_sweep(p_values, q_values, seeds, steps=500)
...
def test_g3_4_4_shock_process_sanity():
    p = 0.05
    q = 0.02
    seed = 1
    steps = 500
    c_spike = 0.05
    shock_sequence = _generate_shock_sequence(q=q, seed=seed, length=steps)
    world = RegimeSwitchBanditWorld(
        p=p, sigma=0.01, seed=seed, q=q, c_spike=c_spike, shock_sequence=shock_sequence
    )
    count_shocks = 0
    for t in range(steps):
        out = world.step(t)
        _, c_total = validate_world_output(out)
        expected = c_spike if shock_sequence[t] else 0.0
        assert c_total == expected
        count_shocks += int(shock_sequence[t])
    shock_rate = count_shocks / float(steps)
    assert abs(shock_rate - q) <= 0.01


def test_g3_4_4_no_semantic_mutation():
    selector = CapitalSelectorBuilder().with_K(0).build()
    stack_manager = StackManager(world_id="regime", phase_id="G3_4_4", run_id="run_0")
    dump_before = canonical_state_dump(selector, stack_manager=stack_manager, sediment=stack_manager.sediment)

    world = RegimeSwitchBanditWorld(p=0.05, sigma=0.01, seed=1, q=0.02, c_spike=0.05)
    out = world.step(0)
