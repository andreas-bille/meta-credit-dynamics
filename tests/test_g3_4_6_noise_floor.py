...
from capitalmarket.capitalselector.experiments.g3_4_6_noise_floor import run_g3_4_6_sweep
...
def test_g3_4_6_determinism():
    p_values = [0.01, 0.05]
    modes = ["stationary", "drift_up"]
    seeds = [0, 1, 2, 3, 4]
    results1 = run_g3_4_6_sweep(p_values, modes, seeds, steps=500)
    results2 = run_g3_4_6_sweep(p_values, modes, seeds, steps=500)
...
def test_g3_4_6_volatility_sanity():
    p = 0.05
    seed = 1
    steps = 500
    ...
    early = residuals[:100].ravel()
    late = residuals[400:].ravel()
    var_early = float(np.var(early))
    var_late = float(np.var(late))
    assert var_late > var_early * (1.0 - 1e-2)


def test_g3_4_6_no_semantic_mutation():
    selector = CapitalSelectorBuilder().with_K(0).build()
    stack_manager = StackManager(world_id="regime", phase_id="G3_4_6", run_id="run_0")
